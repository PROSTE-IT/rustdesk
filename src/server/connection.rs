...
/// No bound of ours once a peer has authorized: `set_max_packet_length` then leaves each
/// transport at its own ceiling.
pub const MAX_AUTHORIZED_MESSAGE: usize = usize::MAX;
...
impl Connection {
    ...
    async fn send_logon_response_and_keep_alive(&mut self) -> bool {
        ...
        self.stream.set_max_packet_length(MAX_AUTHORIZED_MESSAGE);
